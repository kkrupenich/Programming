...
void Pawnshop::takeSmth(ifstream *in)
{
    ...
    while(humanChoice < 0 || humanChoice > humanList.size())
    {
        cout << "Incorrect input, try again: ";
        if(useCin)check(&humanChoice, nullptr);
        else (*in) >> humanChoice;
        
        while(humanList[humanChoice].history != "Empty")
        {
    ...
    while(itemChoice < 0 || itemChoice > itemList.size())
    {
        cout << "Incorrect input, try again: ";
        if(useCin) check(&itemChoice, nullptr);
        else (*in) >> itemChoice;

        while(itemList[itemChoice].humanID != 0)
        {
            cout << "This one already has an owner" << endl;
            cout << "Choose another one: ";
            if(useCin) check(&itemChoice, nullptr);
            else (*in) >> itemChoice;
        }
...
void Pawnshop::returnSmth(ifstream *in)
{
    ...
    while(itemChoice < 0 || itemChoice > itemList.size())
    {
        ...
        while (itemList[itemChoice].humanID != humanList[humanChoice].passport)
        {
